...
import numpy as np
import random
from decimal import * # used for high precision calculations (change made in May 2013)
import sys

from fileoutput import *


getcontext().prec = 12                  # get high precision
getcontext().Emax = 9999999999999999999 # needed to make the limit higher to allow for a couple of really nasty cases
...
def pyes( x, a, m, k ):
    ...
    # print x,a,m,k
    try:
        val= a+(1-a)*(1/(1+Decimal(-k*(x-m)).exp()))
        return val
    except Overflow:
        # print "Warning: emax, setting to 0"
        # print "emax",
        # sys.stdout.flush()
        return Decimal('0')
...
class MLP:
    ...
    def init_hypotheses(self,task):
        ...
        self.hypotheses = [ (Decimal('%.2f'%a),Decimal('%.3f'%m),Decimal('1.0'))  # false alarm rate, threshold, and probability (initially just one)
                            for a in task.FALSE_ALARM_RATES
                            for m in THRESHOLD_HYPOTHESES ]

        #self.hypotheses = [ (Decimal(a),Decimal(m),Decimal(1))  # false alarm rate, threshold, and probability (initially just one)
        #                    for a in task.FALSE_ALARM_RATES
        #                    for m in THRESHOLD_HYPOTHESES ]
    ...
    def updatehypotheses( self, x, answer, task ):
        ...
        newhyp = []
        # print "Stim",x,"answer",answer # for debug
        for (a,m,p) in self.hypotheses:

            # print "Hyp before",(a,m,p) # for debug

            # Calculate the likelihood of a "yes" response to this stimulus
            obsp = pyes( x, a, m, Decimal(task.SLOPE) )

            # print obsp # for debug
            
            # If this was a no-response, invert the probability
            if not answer:
                obsp = 1-obsp

            # print "Hyp after",(a,m,p*obsp) # for debug
    ...
    def getmaximumlikelihood(self):
        ...
        # print maxlikelihyps # for debug
        
        # Now here is an interesting possible case. What if there are 
        # several psychometric curves that each make the data equally likely?
        # This can happen due to rounding and indeed it actually happens very often.
        # Massimo Grassi in his Matlab implementation just chooses the first one (which
        # I think boils down to taking the lowest midpoint and lowest false alarm rate).
        n_maxi_likely = len(maxlikelihyps)

        maxli_hyp = (min([ a for (a,m,p) in maxlikelihyps ]),
                     min([ m for (a,m,p) in maxlikelihyps ]),
                     maxp)


        # If there are several (due to being practically equal), just choose a random one
        # among them
        return (n_maxi_likely,  # how many hypothesis are (equally) maximally likely
                maxli_hyp,      # the minimum FA rate and the minimum midpoint
                )
    ...
    def getsweetpoint( self,
        ...
        y = ((Decimal('1')-a)/(Decimal(p)-a))-Decimal('1')
        stimlevel = (y.ln()/Decimal(-task.SLOPE))+m
    ...
    def run(self, 
        ...
        for i in range(len(self.trials)):
            ...
            (n_optimal_candidates,likelihoodestimate) = self.getmaximumlikelihood()

            # print n_optimal_candidates,likelihoodestimate
        ...
        print "In the last estimate there were %i maximum likelihood curves."%n_optimal_candidates
